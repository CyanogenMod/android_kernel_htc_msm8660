...
static int mmc_wimax_sdio_interrupt_log_status = 0; /* Defautl isn't dumped GPIO wakeup */
...
void mmc_wimax_set_FWWakeupHostEvent(int on) /* RX wakeup */
{
	// printk(KERN_INFO "[WIMAX] %s on:%d\n", __func__, on);
	mmc_wimax_FWWakeupHostEvent = on;
}
EXPORT_SYMBOL(mmc_wimax_set_FWWakeupHostEvent);

int mmc_wimax_get_FWWakeupHostEvent(void)
{
	return mmc_wimax_FWWakeupHostEvent;
}
EXPORT_SYMBOL(mmc_wimax_get_FWWakeupHostEvent);

void mmc_wimax_set_HostWakeupFWEvent(int on) /* TX wakeup */
{
	// printk(KERN_INFO "[WIMAX] %s on:%d\n", __func__, on);
	mmc_wimax_HostWakeupFWEvent = on;
}
EXPORT_SYMBOL(mmc_wimax_set_HostWakeupFWEvent);

int mmc_wimax_get_HostWakeupFWEvent(void)
{
	return mmc_wimax_HostWakeupFWEvent;
}
EXPORT_SYMBOL(mmc_wimax_get_HostWakeupFWEvent);
