		...
		if (i % width == 0) {
			...
			if (len_ >= 34 && !bHandle) { /* DNS [ */
				...
				if ((((unsigned char *)(data))[i+12] == 0x08) &&
					...
					// Andrwe 0306 for parse Query name [
					pos = 39;
			        i =  (unsigned int) (((unsigned char *)(data))[pos]);
					// i = 48;
					dnsLength = i - 24; /* DNS query length */

                    printk(KERN_INFO "%s: Query[i:%d dnsLength:%d]: ", SQN_PRT_MODULE_NAME, i, dnsLength);

					for (pos = 54; pos < (54+dnsLength); pos++) {
					    if (pos < (54+dnsLength-1) )
							printk("%c", ((unsigned char *)(data))[pos]);
						else
							printk("%c\n", ((unsigned char *)(data))[pos]);
					}
					// ]
